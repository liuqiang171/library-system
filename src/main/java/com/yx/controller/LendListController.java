...
public class LendListController {
    ...
    public DataInfo addLend(String readerNumber,String ids,Integer lendDays){
        //获取图书id的集合
        List<String> list= Arrays.asList(ids.split(","));
        //设置应该还书的时间
        long time = new Date().getTime() + lendDays*24*60*60*1000;
        //判断卡号是否存在
        ReaderInfo reader=new ReaderInfo();
        reader.setReaderNumber(readerNumber);
        ...
        if(pageInfo.getList().size()==0){
            ...
            for(String bid:list) {
                ...
                if(bookInfo.getStatus() == 1) {
                    return DataInfo.fail("该图书已经借出，借阅失败");
                }
                ...
                lendList.setDueDate(new Date(time));
    ...
    @ResponseBody
    @GetMapping("/lendListByOrderId")
    public DataInfo lendListByOrderId(@RequestParam(value = "rid", required = false) Integer rid,
                                      @RequestParam(value = "bookName", required = false ) String bookName,
                                      @RequestParam(value = "status", required = false) String status,
                                      @RequestParam(value = "page",defaultValue = "1") Integer page,
                                      @RequestParam(value = "limit",defaultValue = "15") Integer limit){
        PageInfo<LendList> pageInfo = lendListService.queryLendListByReaderId(rid, page, limit,bookName,status);
        return DataInfo.ok("ok",pageInfo.getTotal(),pageInfo.getList());

    }

    @GetMapping("/myBookLend")
    public String myBookLend(){
        return "lend/myBookLend";
    }

    @GetMapping("/renewPage")
    public String renewPage(@RequestParam("id")Integer id, Model model){
        LendList lendList = lendListService.queryLendListById(id);
        model.addAttribute("lendList",lendList);
        return "renew/renewPage";
    }

    @RequestMapping("/renewSubmit")
    @ResponseBody
    public DataInfo renewSubmit(@RequestParam(value = "id") Integer id,
                                @RequestParam("renewDays") Integer renewDays,
        ...
        renew.setLendList(lendList);
        renew.setRenewDays(renewDays);
        renew.setRemarks(remarks);
        renew.setBookInfo(bookInfo);
        renew.setReaderInfo(readerInfo);

        boolean b = renewService.saveRenew(renew);
        ...
        if(b == true){
            ...
            return DataInfo.fail("续借申请失败，请稍后再试");
        }
    }
