...
public class LendListServiceImpl implements LendListService {
    ...
    @Override
    public PageInfo<LendList> queryLendListByReaderId(Integer rid, Integer page, Integer limit, String bookName, String status) {
        PageHelper.startPage(page,limit);
        if(!StringUtils.isEmpty(bookName)){
            bookName = bookName.trim();
        }
        List<LendList> lendLists = lendListMapper.queryLendListByReaderId(rid,bookName,status);
        PageInfo<LendList> pageInfo = new PageInfo<>(lendLists);
        return pageInfo;
    }

    @Override
    public LendList queryLendListById(Integer id) {

        return lendListMapper.queryLendListById(id);
    }

    @Override
    public boolean updateById(LendList lendList) {

        return lendListMapper.updateById(lendList);
    }

    @Override
    public boolean updateDueDate(LendList lendList) {
        return lendListMapper.updateDueDate(lendList);
    }
