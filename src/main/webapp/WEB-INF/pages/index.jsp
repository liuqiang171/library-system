...
<script>
    layui.use(['jquery', 'layer', 'miniAdmin','miniTongji'], function () {
        ...
        var options = {
            <c:choose>
                <c:when test="${sessionScope.type.equals('admin') && sessionScope.user.adminType == 0}">
                    iniUrl: "${pageContext.request.contextPath}/api/init3.json",    // 初始化接口，普通管理员
                </c:when>
                <c:when test="${sessionScope.type.equals('admin') && sessionScope.user.adminType == 1}">
                    iniUrl: "${pageContext.request.contextPath}/api/init.json",    // 初始化接口，高级管理员
                </c:when>
                <c:otherwise>
